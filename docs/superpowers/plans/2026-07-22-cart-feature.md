...
cartItemRoute
  .post(
    "/",
    validate(cartItemSchema.createCartItem),
  ...
  .put(
    "/:id",
    validate(cartItemSchema.updateCartItem),
          ...
          include: {
            product_variant: {
              include: {
                ...
                VariableAttributes: {
                  include: {
                    attributeKey: { select: { name: true, unit: true } },
                  },
                },
              },
            },
...
const cartReducer = (state, action) => {
  switch (action.type) {
    ...
    case "ADD_ITEM": {
      ...
      return [
        ...state,
        { product_variant_id, quantity, product, variant, cart_item_id: null },
      ];
    }
    case "UPDATE_QTY":
      return state.map((i) =>
...
export const CartProvider = ({ children }) => {
  ...
  const addItem = useCallback(
    async (product_variant_id, quantity = 1, product, variant) => {
      dispatch({
        type: "ADD_ITEM",
        payload: { product_variant_id, quantity, product, variant },
      });
      if (user?.id) {
        try {
  ...
  const updateQty = useCallback(
    async (product_variant_id, quantity) => {
      dispatch({
        type: "UPDATE_QTY",
        payload: { product_variant_id, quantity },
      });
      if (user?.id) {
        try {
  ...
  const totalAmount = items.reduce(
    (sum, i) =>
      sum + (i.variant?.price || i.product?.base_price || 0) * i.quantity,
    0,
  );
  ...
  return (
    <div className="flex items-start gap-4 p-4 bg-white border border-slate-200 rounded-sm">
      ...
      <div className="flex items-center border border-slate-300 rounded-sm overflow-hidden shrink-0">
        <button
          onClick={() =>
            onUpdateQty(item.product_variant_id, Math.max(1, item.quantity - 1))
          }
      ...
      <p className="text-sm font-bold text-slate-800 w-24 text-right shrink-0">
...
_Không cần thay đổi thêm — CartContext đã xử lý việc này._
