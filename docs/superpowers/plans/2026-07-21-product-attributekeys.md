    ...
    return prisma.ProductAttributeKeys.update({
      ...
      data: {
        ...
        ...(data.attribute_key_id && {
          attribute_key_id: parseInt(data.attribute_key_id),
        }),
      },
      include: {
        product: { select: { id: true, name: true } },
...
const productAttributeKeyController = {
  ...
  getAll: async (req, res) => {
    try {
      ...
      const result = await productAttributeKeyService.getAll({
        page,
        product_id,
      });
      ...
      if (!result || result.data.length === 0) {
        return res
          .status(404)
          .json({ success: false, message: "Không tìm thấy bản ghi nào." });
      }
  ...
  update: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, data: result, message: "Cập nhật thành công" });
    } catch (error) {
      if (error.code === "P2025")
...
productAttributeKeyRoute
  .post(
    "/",
    validate(productAttributeKeySchema.create),
    productAttributeKeyController.create,
  )
  ...
  .put(
    "/:id",
    validate(productAttributeKeySchema.update),
    productAttributeKeyController.update,
  )
...
app.use(
  `${api_prefix_v1}management/product-attribute-key/`,
  productAttributeKeyRoute,
);
```
...
const productAttributeKeyApi = {
  getAll: (params = {}) => {
    ...
    return axiosClient.get(
      `/management/product-attribute-key?${query.toString()}`,
    );
  },
  ...
  getByProduct: (productId) => {
    return axiosClient.get(
      `/management/product-attribute-key/by-product/${productId}`,
    );
  },
  ...
  getAll: async ({ page = 1, product_id = "" } = {}) => {
    ...
    try {
      const response = await axiosClient.get(
        `management/product-attribute-key?${params.toString()}`,
      );
      return response;
    } catch (error) {
...
const ProductAttributeKey = () => {
  ...
  useEffect(() => {
    LoaderProductAttributeKey.getAll({
      page: currentPage,
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.success) {
        ...
        const res = await LoaderProductAttributeKey.getAll({
          page: currentPage,
          product_id: currentProductId,
        });
        if (res?.success) {
                ...
                data.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      {item.attributeKey?.name}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <Badge color="blue">
                        {item.attributeKey?.unit || "—"}
                      </Badge>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <BtnActions
                        ...
                        id={item.id}
                        onDelete={() =>
                          openConfirm(item.id, item.attributeKey?.name)
                        }
                      />
                    </td>
                  </tr>
