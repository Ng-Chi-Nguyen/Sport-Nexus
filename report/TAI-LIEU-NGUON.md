...
|------|-----------|---------|
| 1 | Đọc `server/prisma/schema.prisma` | Mô hình dữ liệu đầy đủ 22 bảng |
...
**Trong SportNexus:** toàn bộ 22 bảng được khai báo trong `server/prisma/schema.prisma`, và mọi thao tác dữ liệu trong service đều dùng Prisma Client.
...
Lưu trữ toàn bộ dữ liệu nghiệp vụ. 22 bảng, được mô tả chi tiết ở Chương 5.
...
Hệ thống gồm **22 bảng** chia thành các nhóm nghiệp vụ: Identity, Catalog, Sản phẩm, Bán hàng, Thanh toán, Vận chuyển, Kho, và Audit. Dưới đây trình bày chi tiết từng bảng cùng **lý do thiết kế**.
...
Chương này đã trình bày chi tiết 22 bảng của cơ sở dữ liệu SportNexus, chia thành 8 nhóm nghiệp vụ, cùng các nguyên tắc thiết kế (chuẩn hóa, khóa ngoại, Decimal, enum, soft delete, EAV, Json, chỉ mục). Thiết kế này đáp ứng đầy đủ các yêu cầu chức năng đã phân tích ở Chương 3 và là nền tảng cho việc triển khai backend ở Chương 6.
...
function Button({ variant, size, children, ...props }) {
  return <button className={clsx(styles.base, styles[variant], styles[size])} {...props}>{children}</button>;
}
```
...
| CSDL quan hệ chuẩn hóa | Đạt — 22 bảng, soft delete |
