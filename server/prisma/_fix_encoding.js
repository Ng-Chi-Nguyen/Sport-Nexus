...
const filePath = 'D:\\Programming\\SportNexus\\client\\src\\pages\\Admin\\Dashboard\\inventory\\MovementStats.jsx';

const content = import { Card } from "@/pages/Admin/Dashboard/components/Card";
...
export const MovementStats = ({ movementCountByType = {}, movementTrend = [] }) => {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2 items-start">
      <Card title="Số lần biến động theo loại" icon={<ArrowUpDown size={16} />}>
        {hasCounts ? (
              ...
              return (
                <div key={type} className="flex items-center justify-between rounded-xl border border-slate-800 bg-[#08101E] px-3 py-2">
                  <div className="flex items-center gap-2">
                    <div className={\lex h-7 w-7 items-center justify-center rounded-lg \ \\}>{meta.icon}</div>
                    <div>
                  ...
                  <div className="text-right">
                    <p className={\	ext-lg font-bold \\}>{count}</p>
                  ...
                  <div key={item.period} className="group relative flex min-w-[32px] flex-1 flex-col items-center">
                    ...
                    <div className="flex h-[160px] w-full items-end rounded-lg border border-slate-800/80 bg-[#08101E] p-0.5 gap-px">
                      <div className="flex w-1/3 flex-col-reverse items-center h-full">
                        <div className="w-full rounded-sm bg-emerald-500/70 transition-all" style={{ height: \\%\ }} />
                      </div>
                      <div className="flex w-1/3 flex-col-reverse items-center h-full">
                        <div className="w-full rounded-sm bg-red-500/70 transition-all" style={{ height: \\%\ }} />
                      </div>
                      <div className="flex w-1/3 flex-col-reverse items-center h-full">
                        <div className="w-full rounded-sm bg-amber-500/70 transition-all" style={{ height: \\%\ }} />
                      </div>
                    </div>
...
console.log('MovementStats.jsx written successfully with correct UTF-8');
